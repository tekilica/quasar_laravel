        ...
        <table class="w-full table-auto">
            ...
            <tbody>
                @foreach($videos as $video)
                    <input type="hidden" name="ids[]" value="{{ $video->id }}" form="edit-form">

                    <tr>
                        <td class="px-[16px] py-[8px] {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }} border-t">
                            <video class="h-[128px] mx-auto rounded object-contain" src="{{ asset('storage/videos/'.$video->file) }}"></video>
                        </td>

                        <td class="px-[16px] py-[8px] {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }} border-t border-l">
                            <input class="w-full rounded-full" type="text" name="name[]" value="{{ $video->name }}" form="edit-form">
                        </td>

                        <td class="px-[16px] py-[8px] {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }} border-t border-l">
                            <textarea class="w-full rounded" rows="3" name="description[]" form="edit-form">{{ $video->description }}</textarea>
                        </td>
                    </tr>
                @endforeach
            </tbody>
