...
@section('content')
    ...
    <div class="movable flex flex-row z-10 fixed h-[80px] top-[96px] right-[32px] p-[16px] rounded shadow bg-white transition-all duration-500 justify-end items-center" style="left: 96px;">
        ...
        <button class="flex flex-row ml-[16px] px-[16px] py-[8px] rounded-full border shadow border-gray-500 bg-white text-gray-500 hover:bg-gray-500 hover:text-white transition-all duration-150 justify-center items-center" type="button" onclick="window.location.href = '{{ redirect('/soundtracks')->getTargetUrl() }}'">
        ...
        <table class="w-full table-auto">
            ...
            <tbody>
                @foreach($soundtracks as $soundtrack)
                    <input type="hidden" name="ids[]" value="{{ $soundtrack->id }}" form="edit-form">
                    ...
                    <tr>
                        ...
                        <td class="px-[16px] py-[8px] {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }} border-t border-l">
                            <input class="w-full rounded-full" type="text" name="name[]" value="{{ $soundtrack->name }}" form="edit-form">
                        </td>

                        <td class="px-[16px] py-[8px] {{ $loop->odd ? 'bg-gray-50' : 'bg-white' }} border-t border-l">
                            <textarea class="w-full rounded" rows="3" name="description[]" form="edit-form">{{ $soundtrack->description }}</textarea>
                        </td>
                    </tr>
                @endforeach
            </tbody>
